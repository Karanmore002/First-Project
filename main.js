...
road2.position.set(850, 0, -150); // Adjust position to align with curved road
road2Geometry.parameters.width = 450; // Match the width of the curved road's outer radius
road2Texture.repeat.set(10, 1); // Adjust texture repeat for smoother appearance
scene.add(road2);
...
function createLeftCurveRoad() {
  ...
  curveTexture.repeat.set(10, 1); // Adjust texture repeat for consistency

  const curveMaterial = new THREE.MeshStandardMaterial({ map: curveTexture });

  const leftCurveGeometry = new THREE.RingGeometry(
    450, // Inner radius - Adjusted for better compatibility
    500, // Outer radius
    32,  // Increase segments for smoother curvature
    8,   // Number of rings
    -Math.PI / 3, // Start angle to create a smooth arc
    Math.PI / 3    // Sweep angle for a tighter curve
  );
  const leftCurve = new THREE.Mesh(leftCurveGeometry, curveMaterial);
  leftCurve.rotation.x = -Math.PI / 2; // Lay flat
  leftCurve.position.set(1250, 0, 0); // Align the curve with road2's end
  return leftCurve;
}


function createCurvedSeparator() {
  const separatorMaterial = new THREE.MeshStandardMaterial({ color: 0x333333 });
  const curvedSeparatorGeometry = new THREE.RingGeometry(
    395, // Inner radius slightly smaller than curved road
    405, // Outer radius slightly larger than curved road
    32,  // Increase segments for smoother curve
    8,   // Number of rings
    -Math.PI / 2, 
    Math.PI / 2
  );
  const curvedSeparator = new THREE.Mesh(curvedSeparatorGeometry, separatorMaterial);
  curvedSeparator.rotation.x = -Math.PI / 2;
  curvedSeparator.position.set(1250, 0.01, -150); // Slightly raised to avoid z-fighting
  return curvedSeparator;
}

// Add the curved separator to the scene
const curvedSeparator = createCurvedSeparator();
scene.add(curvedSeparator);
